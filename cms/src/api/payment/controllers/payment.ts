...
export default {
          ...
          await knex.transaction(async (trx) => {
            ...
            const rbr = await trx.raw(
              ...
              select
                ...
                br.email,
                l.boat_id,
                coalesce(b.instant_booking,false) as instant_booking
              from public.booking_requests br
            ...
            const isInstantBooking = Boolean(brRow?.instant_booking);
            const shouldPendingOwner = !isInstantBooking && allowBeforeOwner && (brSt === "new" || brSt === "pending");
  ...
  async webhook(ctx) {
        ...
        if (dodoSignature) {
          ...
          if (!dodoSecret) {
            ctx.status = 503;
            ctx.body = { error: "dodo_webhook_secret_missing" };
            return;
          }
          ...
          let paidNotifyBookingRequestId: number | null = null;
          ...
          await strapi.db.connection.transaction(async (trx) => {
            ...
            const payment =
              (payRes && ((payRes.rows && payRes.rows[0]) || (Array.isArray(payRes[0]) ? payRes[0][0] : null))) as any;

            if (!payment) {
              return;
            }

            const brId = payment.booking_request_id;
            paidNotifyBookingRequestId = Number(brId) || null;
            if (!brId) {
              await trx.raw(
                "update public.payments set status = 'succeeded', metadata = coalesce(metadata, '{}'::jsonb) || ?::jsonb, updated_at = now() where id = ?",
                [JSON.stringify({ last_event_type: eventType, provider_status: providerStatus, webhook_received_at: new Date().toISOString(), missing_booking_request_id: true }), payment.id]
              );
              return;
            }

            const brRes = await trx.raw(
              `
              select
                br.id as booking_request_id,
                br.status as br_status,
            ...
            const brRow =
              (brRes && ((brRes.rows && brRes.rows[0]) || (Array.isArray(brRes[0]) ? brRes[0][0] : null))) as any;

            if (!brRow) {
              await trx.raw(
                "update public.payments set status = 'succeeded', metadata = coalesce(metadata, '{}'::jsonb) || ?::jsonb, updated_at = now() where id = ?",
                [JSON.stringify({ last_event_type: eventType, provider_status: providerStatus, webhook_received_at: new Date().toISOString(), missing_booking_request: true }), payment.id]
              );
              return;
            }

            const isInstantBooking = Boolean(brRow.instant_booking);
            const finalStatus = isInstantBooking ? "confirmed" : "paid_pending_owner";

            const bookingRes = await trx.raw(
              `
              ...
              [
                finalStatus,
                providerIntentId,
                Number(payment.amount_cents || 0) / 100.0,
                String(payment.currency || "EUR"),
                String(brRow.full_name || ""),
                String(brRow.phone || ""),
                String(brRow.email || ""),
                finalStatus,
                Number(brRow.boat_id),
                brRow.start_datetime,
                brRow.end_datetime,
              ]
            );

            const booking =
              (bookingRes && ((bookingRes.rows && bookingRes.rows[0]) || (Array.isArray(bookingRes[0]) ? bookingRes[0][0] : null))) as any;
            ...
            if (!booking?.id) {
              ...
              await trx.raw(
                "update public.booking_requests set status = 'paid_needs_review', updated_at = now() where id = ? and lower(coalesce(status,'')) not in ('declined','cancelled','canceled','confirmed')",
                [brId]
              );
            ...
            await trx.raw(
              "update public.payments set status = 'succeeded', booking_id = coalesce(booking_id, ?), metadata = coalesce(metadata, '{}'::jsonb) || ?::jsonb, updated_at = now() where id = ?",
              [booking.id, JSON.stringify({ last_event_type: eventType, provider_status: providerStatus, webhook_received_at: new Date().toISOString(), instant_booking: isInstantBooking, linked_existing_hold: true }), payment.id]
            );

            await trx.raw(
              "update public.booking_requests set status = ?, updated_at = now() where id = ? and lower(coalesce(status,'')) not in ('declined','cancelled','canceled')",
              [finalStatus, brId]
            );
          });

          try {
            const notifyUrl = String(process.env.SHARMAR_FRONTEND_NOTIFY_URL || "").trim();
            const notifySecret = String(process.env.SHARMAR_INTERNAL_NOTIFY_SECRET || "").trim();
            ...
            if (notifyUrl && notifySecret) {
              const notifyRes = await fetch(notifyUrl, {
                ...
                headers: {
                  "Content-Type": "application/json",
                  "X-Sharmar-Internal-Secret": notifySecret,
                },
                body: JSON.stringify({
                  booking_request_id: paidNotifyBookingRequestId,
              ...
              if (!notifyRes.ok) {
                strapi.log.warn(`DODO_PAID_NOTIFY_FAILED status=${notifyRes.status}`);
              } else {
                strapi.log.info(`DODO_PAID_NOTIFY_OK provider_intent_id=${providerIntentId}`);
              }
            } else {
              strapi.log.warn("DODO_PAID_NOTIFY_SKIPPED missing_notify_env");
            }
          } catch (e) {
            strapi.log.warn(`DODO_PAID_NOTIFY_ERROR ${e instanceof Error ? e.message : String(e)}`);
          }

          ctx.status = 200;
          ctx.body = { ok: true, provider: "dodo", provider_intent_id: providerIntentId };
          return;
        }
...
// Authorized/manual-capture flow:
// Stripe amount_capturable_updated means the customer authorized payment.
// At this point we must link the existing hold to the payment and wait for owner confirmation.
...
if (newStatus === "authorized") {
  await strapi.db.connection.raw(
    ...
    upd_booking as (
      update public.bookings
         set status = 'paid_pending_owner',
             payment_intent_id = (select provider_intent_id from p),
             deposit_amount = (select coalesce(amount_cents, 0)::numeric / 100.0 from p),
             currency = (select coalesce(nullif(currency,''),'EUR') from p)
       where id = (select id from b)
       returning id
    ),
    upd_payment as (
      update public.payments
         set booking_id = coalesce(booking_id, (select id from upd_booking)),
             updated_at = now()
       where provider = 'stripe'
         and provider_intent_id = ?
       returning id
    ...
    update public.booking_requests
       set status = 'paid_pending_owner',
           updated_at = now()
     where id = (select booking_request_id from p)
       and lower(coalesce(status,'')) in ('new','pending')
       and exists (select 1 from upd_booking)
    `,
    [intentId, intentId]
  );
}
...
await trx.raw(
        ...
        if (providerStatus === "succeeded") newStatus = "succeeded";
